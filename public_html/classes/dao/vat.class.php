...
include_once(ROOT_PATH."classes/dao/vatinfo.class.php");
...
class Vat extends Model {
	...
	public function __construct($store_id = 0, $database = '') {
		...
		$this->table = DB_PREFIX."vat";
		$this->store_id = $store_id;
	}
	public function Vat($store_id = 0, $database = '') {
...
*-----------------------------------------------------------------------*/
	...
	public function getObjectsAllStores($page = 1, $condition = '1>0', $sort = array(), $items_per_page = DEFAULT_ADMIN_ROWS_PER_PAGE) {
		if(!$page) $page = 1;
		$start = ($page -1) * $items_per_page;
...
*-----------------------------------------------------------------------*/	
	...
	# Update record
	public function updateDataAllStores($fields, $value = '', $key = 'id') {
	...
	public function getVatFromId($id='') {
		if(!$id) return '';
		$result = $this->select('vat',"`store_id` = '".$this->store_id."' AND id = '$id'");
		if($result) return $result[0]['vat'];
		return '';
	}
...
*------------------------------------------------------------------------*/
	...
	# Return a Vat value name from VAT
	public function getVatValueFromVat($vat='') {
		if(!$vat) return '';
		$result = $this->select('vat_value',"`store_id` = '".$this->store_id."' AND vat = '$vat'");
		if($result) return $result[0]['vat_value'];
		return '';
	}

	# Return a Article name from provided ID
	public function getVatFromVatValue($vat_value='') {
		if(!$vat_value) return '';
		$result = $this->select('vat',"`store_id` = '".$this->store_id."' AND `properties` LIKE '%$vat_value%' AND `properties` NOT LIKE '%KKKNT%'");
		if($result) return $result[0]['vat'];
		return '';
	}
