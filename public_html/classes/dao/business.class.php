...
class Business extends Model {
	...
	public function __construct($store_id = 0, $database = '') {
		...
		$this->table = DB_PREFIX."businessbranch";
		$this->store_id = $store_id;
	}
	public function Business($store_id = 0, $database = '') {
	...
	public function getObject($value = '0', $key = 'id', $condition = '1>0') {
		...
		if($result) {
			$object = new BusinessInfo
						(	
							$result[0]['codoBusiness'],
							$result[0]['nameBusiness'],
							$result[0]['addressBusiness'],
							$result[0]['properties'],
...
*-----------------------------------------------------------------------*/	
	...
	public function changeDefault($id = 0, $status = '') {
		if(!$id) return 0;
		if($this->update(array('default' => $status), "`store_id` = '".$this->store_id."' AND `id` = '$id'")) return 1;
		return 0;
	}
	public function changeHthuc($id = 0, $hthuc = '') {
		if(!$id) return 0;
		if($this->update(array('hthuc' => $hthuc), "`store_id` = '".$this->store_id."' AND `id` = '$id'")) return 1;
		return 0;
	}
	...
	public function getCksDefault() {
		$result = $this->select('id',"`store_id` = '".$this->store_id."' AND `default` = '1'");
...
*------------------------------------------------------------------------*/
	...
	public function checkDuplicateSerial($value = '', $key = 'serial', $condition = '') {
		$result = $this->select("`$key`","`store_id` = '".$this->store_id."' AND `$key` = '$value'".($condition?" AND $condition":''));
		if($result) return 1;
		return 0;
	}

	public function getIdFromCodeBusiness($codeBusiness='') {
		if(!$codeBusiness) return '';
		$result = $this->select('id',"`store_id` = '".$this->store_id."' AND `codoBusiness` = '$codeBusiness'");
		if($result) return $result[0]['id'];
		return '';
	}	
