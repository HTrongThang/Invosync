...
include_once(ROOT_PATH."classes/dao/versioninfo.class.php");
...
class Versions extends Model {
	...
	public function __construct($store_id = 0, $database = '') {
		...
		$this->table = DB_PREFIX."version";
		$this->store_id = $store_id;
	}
	public function Stores($store_id = 0, $database = '') {
...
*-----------------------------------------------------------------------*/	
	...
	# Update record
	public function updateDataVersion($fields, $value = '', $key = 'id') {
...
*------------------------------------------------------------------------*/
	...
	public function checkId($value = '', $key = 'id', $condition = '') {
		$result = $this->select("`$key`","`store_id` = '".$this->store_id."' AND `$key` = '$value'".($condition?" AND $condition":''));
		if($result) return 1;
		return 0;
	}
