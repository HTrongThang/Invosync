...
$topNav = array($amessages['dash_board'] => '/'.ADMIN_SCRIPT.'?op=dashboard',
				...
				'Đăng ký nhận tin tuyển dụng' => '/'.ADMIN_SCRIPT.'?op=manage&act=comment',
				$amessages['list_item'] => '');
...
$condition ="1>0";
if($kw) $condition = "(`id`='$kw' OR `fullname` LIKE '%$kw%' OR `email` LIKE '%$kw%')";
		...
		case 'enable':
			...
			if($id) {
				...
				if($ids) {
					...
					$result_code = 1;
					# Operation tracking
					// $trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['enable_comment'],$listComment),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
				} else $error_code = 5;
			}
			break;
		case 'disable':
			...
			if($id) {
				...
				if($ids) {
					...
					$result_code = 2;
					# Operation tracking
					// $trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['disable_comment'],$listComment),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
				} else $error_code = 5;
			}
			break;
		case 'delete':
			...
			if($id) {
				...
				if($ids) {
					...
					$result_code = 3;
					# Operation tracking
					// $trackings->addData(array('store_id'=>$storeId,'username'=>$userInfo->getUsername(),'action'=>sprintf($amessages['tracking']['delete_comment'],$listComment),'date_created'=>date("Y-m-d H:i:s"),'ip'=>$_SERVER['REMOTE_ADDR']));
				} else $error_code = 5;
			}
			break;
		case 'changeposition':
